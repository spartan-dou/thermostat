...
async def _async_setup_config(
    ...
    cold_tolerance: float = float(config[CONF_COLD_TOLERANCE])
    hot_tolerance: float = float(config[CONF_HOT_TOLERANCE])
...
class CustomGenericThermostat(ClimateEntity, NumberEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self.target_temp_entity_id = None
        ...
        self._target_temp_preset_none = target_temp
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if (old_state := await self.async_get_last_state()) is not None:
            ...
            if (target_temp_preset_none := old_state.attributes.get(ATTR_TARGET_TEMP_PRESET_NONE)) is not None:
                self._target_temp_preset_none = target_temp_preset_none
            else:
                self._target_temp_preset_none = self._target_temp
            if (
                self.preset_modes
    ...
    @property
    def target_temp_preset_none(self):
        """Return the temperature to none mode."""
        return self._target_temp_preset_none
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        if self._attr_preset_mode == PRESET_NONE:
            self._target_temp_preset_none = temperature
    ...
    def extra_state_attributes(self):
        return {
            "target_temp_preset_none": self._target_temp_preset_none,
        }
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        ...
        if preset_mode == PRESET_NONE:
            self._attr_preset_mode = PRESET_NONE
            self._target_temp = self._target_temp_preset_none
            await self._async_control_heating(force=True)
        else:
